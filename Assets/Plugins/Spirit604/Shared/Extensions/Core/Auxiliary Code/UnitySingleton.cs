    ...
    public class UnitySingleton<T> where T : MonoBehaviour
    {
        ...
        static internal void InternalAwake(T instance)
        {
            ...
            if (globalInstanceCount > 1)
            {
...
#endif
                {
                    ...
                    Spirit604.Extensions.CustomDebug.LogWarning("More than one awake of SingletonMonoBehaviour " + typeof(T).Name);
                }
            }
            else
            {
        ...
        static internal void InternalDestroy()
        {
            ...
            else
            {
                ...
                {
                    // can happen if an exception occurred that prevented correct instance counting 
                    Spirit604.Extensions.CustomDebug.LogWarning("More than one destroy of SingletonMonoBehaviour " + typeof(T).Name);
                }
            }
        }
        ...
        static public T GetSingleton(bool autoCreate)
        {
            if (!instanceT) // Unity operator to check if object was destroyed, 
            {
                ...
                if ((Application.isPlaying) && (!UnityEditor.EditorApplication.isPaused))
                {
                    var components = ObjectUtils.FindObjectsOfType<T>();
                    ...
                    foreach (var c in components)
                    {
                        ...
                        {
                            component = c;
                            Spirit604.Extensions.CustomDebug.LogError("Must call base.Awake() singleton : " + typeT.Name);
                            break;
                        }
                    }
                }
                ...
                if (!component)
                {
                    ...
                    )
                    {
                        ...
                        Spirit604.Extensions.CustomDebug.LogWarning("Create singleton : " + typeT.Name);

                        component = go.GetComponent<T>();
                        if (component == null)
                        {
                            Spirit604.Extensions.CustomDebug.LogError("Auto created object does not have component " + typeT.Name);
                            component = null;
                        }
                    }
                    else
